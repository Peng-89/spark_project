...
/**
  ...
  */
object SparkSaveDetail {
  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setAppName("SparkSaveDetail")
    ...
    val detail = sqlContext.read.format("es").load("company_detail/detail").distinct()
      .withColumnRenamed("item","item_detail")


    val joindata = resultdistinct.join(detail,resultdistinct.col("item").equalTo(detail.col("item_detail")))

    joindata.repartition(10).write.format("com.databricks.spark.csv")
      .option("header", "true").mode(SaveMode.Overwrite).save("hdfs://master:9000/user/data/company_detail")
